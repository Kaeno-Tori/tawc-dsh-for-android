...
- `proxy_ssl_server_name on; proxy_ssl_name $2;` — variable-based
  `proxy_pass` defaults to no SNI, so Fastly-fronted mirrors reply 421
  Misdirected Request because
  the served default cert doesn't cover the requested Host. Forcing
  SNI to match the proxied hostname fixes it.
- Range requests aren't cached by stock nginx. apt/pacman fall
...
2. Forwards `mirrorProxy` to `Distro.configure`, which rewrites
   ...
   - **Arch:** `Server = <url>` lines in
     `/etc/pacman.d/mirrorlist` (handled in
     `ArchPacmanCommon.configure` via the `SERVER_LINE_RE` regex).
   - **Debian:** the `URIs:` line in
     `/etc/apt/sources.list.d/tawc.sources` (handled in
     `AptCommon.configure`).
   - Pacman's `$repo`/`$arch` substitutions happen after URL
     composition, so dollar signs survive
...
- Rootfs tarballs (Arch ALARM, Arch x86, Debian) — the
  giant initial download.
- Per-package files (`.pkg.tar.zst`, `.deb`).
...
Verification fetches (PGP `.sig`, GitHub Releases REST API) **do**
route through the proxy in dev builds. Keeping
them off-proxy produced a permanent verification mismatch the moment
upstream rotated an artifact (proxy serves stale tarball, fresh digest
fetched direct → mismatch with no in-band recovery). Both Arch
bootstraps are unaffected either way: a `.sig` is still checked against
a key in the APK, so a proxy that tampers with it fails closed. The
digest-only distros do lean on the proxy in dev; release builds never
construct a `MirrorProxy`, so the upstream-direct story is intact for
users.
...
- The mirror config baked into the rootfs hard-codes the
  ...
  apt inside the chroot **without** the proxy running will fail.
...
- Range requests (HTTP 206) aren't cached because stock nginx
  doesn't slice on `bytes=` — apt/pacman don't use them in
  practice, so this only matters for ad-hoc curl tests.

## Measured speedup

Verification run (Manjaro/tawcroot on physical aarch64 — recorded
before Manjaro ARM was dropped from this fork, so the package counts
are that old rootfs's; `mirror.alwyzon.net`
